...
using namespace std;
//xinw added for generating trace-begin
#include <fstream>
/*Tick last_request_tick=0;
struct _trace_entry
{
...
//xinw added for generating trace-end
...
int simulation_mode=0;
    ...
    virtual uint64_t get_remaining_queue_size(Request req)=0;
    virtual uint64_t get_overflow_queue_size(Request req)=0;
    virtual uint64_t prioritize(Request req)=0;
    //xinw added for morphable-end
    virtual void set_warmuptime(Tick _atomicwarmuptime, Tick _warmuptime)=0;
    virtual void set_simulation_mode(int detailed_mode)=0;
    virtual void set_timeout(uint64_t _timeout)=0;
...
public:
    ...
    {
	//xinw added for generating trace-begin
	outputDir = configs.get_tracefile_directory();
	trace_name = configs.get_trace_name();
	generate_trace = configs.get_generate_trace();
	//xinw added for generating trace-begin
...
bool send(Request req)
	...
	/*if (req.type == Request::Type::READ) {
		ctrls[0]->counter_read_num++;
	}else if (req.type == Request::Type::WRITE) {
		ctrls[0]->counter_write_num++;
	}*/	
      }
      else{
	req.is_counter=false;
	/*	
	if (req.type == Request::Type::READ) {
		ctrls[0]->data_read_num++;
	}else if (req.type == Request::Type::WRITE) {
		ctrls[0]->data_write_num++;
	}*/	
      }

      //xinw added 
	//if(curTick()<(begin_tick+atomicwarmuptime))
	if(!simulation_mode)
	{
		req.callback(req);
		return true;
      ...
      //  long addr = req.addr;
         long addr;
        if(req.relocated_addr)
        {
                addr =req.addr;
                //addr = req.relocated_addr;
                //printf("relocated_addr: %ld\n", addr);
        }
        else
        {
                addr = req.addr;
                //printf("original addr: %ld\n", addr);
        }
	...
	int column, channel, bank, bankgroup, rank, row, tempA, tempB, channel_mask, channel_mask_temp;
      long addr_copy;
      int bankgroup_bit0, bankgroup_bit1;
		...
		*/
		...
		channel = slice_lower_bits(addr, addr_bits[0]);
		bankgroup_bit0 = slice_lower_bits(addr, addr_bits[2] >> 1);
		column = slice_lower_bits(addr, addr_bits[addr_bits.size() - 1]);
		bankgroup_bit1 = slice_lower_bits(addr, addr_bits[2] >> 1);
		bank = slice_lower_bits(addr, addr_bits[3]);
		rank = slice_lower_bits(addr, addr_bits[1]);
		row = slice_lower_bits(addr, addr_bits[4]);
		...
		tempB = (row >> addr_bits[3]) & ((1 << (addr_bits[2]) >> 1) - 1);

		bank = bank ^ tempA;
		bankgroup_bit1 = bankgroup_bit1 ^ tempB;

		bankgroup = bankgroup_bit0 + (bankgroup_bit1 << (addr_bits[2] >> 1));
		*/

		tempA = row & 0x1;
		tempB = (row >> 0x1) & ((1 << addr_bits[3]) - 1);

		bankgroup_bit1 = bankgroup_bit1 ^ tempA;
		bankgroup = bankgroup_bit0 + (bankgroup_bit1 << (addr_bits[2] >> 1));
		...
		*/
		break;
	    case int(Type::skylake):
		addr_copy = addr << tx_bits;
		column = slice_lower_bits(addr, addr_bits[addr_bits.size() - 1]);
		if(addr_bits[0] == 1)
		  channel = get_bit_at(addr_copy, 8) ^ get_bit_at(addr_copy, 9) ^ get_bit_at(addr_copy, 12) ^ get_bit_at(addr_copy, 13) ^ get_bit_at(addr_copy, 18)  ^ get_bit_at(addr_copy, 19);
		else if(addr_bits[0] == 2)
		  {
		    int channel_bit0, channel_bit1;
		    channel_bit0 = get_bit_at(addr_copy, 8) ^ get_bit_at(addr_copy, 12) ^ get_bit_at(addr_copy, 13) ^ get_bit_at(addr_copy, 18)  ^ get_bit_at(addr_copy, 19);
		    channel_bit1 = get_bit_at(addr_copy, 9) ^ get_bit_at(addr_copy, 12) ^ get_bit_at(addr_copy, 13) ^ get_bit_at(addr_copy, 18)  ^ get_bit_at(addr_copy, 19);
		    channel = channel_bit0 + (channel_bit1 << 1);
		  }
		else if(addr_bits[0] > 2)
		  {
		    int channel_bit0, channel_bit1, channel_bit2;
		    channel_bit0 = get_bit_at(addr_copy, 8);
		    channel_bit1 = get_bit_at(addr_copy, 9);
		    //channel_bit2 = get_bit_at(addr_copy, 13);
		    //xinw changed bit 13 to bit 10
		    channel_bit2 = get_bit_at(addr_copy, 10);
		    channel = channel_bit0 + (channel_bit1 << 1) + (channel_bit2 << 2);
		  }

		else
		  channel = 0;

		//bankgroup_bit0 = get_bit_at(addr_copy, 7) ^ get_bit_at(addr_copy, 14);
		//xinw changed for better bandwidth utilization
		bankgroup_bit0 = get_bit_at(addr_copy, 6) ^ get_bit_at(addr_copy, 14);

		bankgroup_bit1 = get_bit_at(addr_copy, 15) ^ get_bit_at(addr_copy, 19);
		bankgroup = bankgroup_bit0 + (bankgroup_bit1 << 1);

		//rank = get_bit_at(addr_copy, 16) ^ get_bit_at(addr_copy, 20);
		//daz3: to use 4 ranks
		if(addr_bits[1] == 1)   // gagan: 2 rank, rank bits = 1
			rank = get_bit_at(addr_copy, 16) ^ get_bit_at(addr_copy, 20);
		else if(addr_bits[1] == 2)   // gagan: 4 ranks, rank bits = 2
			rank = get_bit_at(addr_copy, 16) + (get_bit_at(addr_copy, 20) << 1);
		else if(addr_bits[1] == 3) // gagan, xinw: 8 ranks, rank bits = 3
			rank = get_bit_at(addr_copy, 13) + (get_bit_at(addr_copy, 16) << 1) + (get_bit_at(addr_copy, 20) << 2);
		else if(addr_bits[1] == 4) // gagan, xinw: 16 ranks, rank bits 4
			rank = get_bit_at(addr_copy, 12) + (get_bit_at(addr_copy, 13) << 1) + (get_bit_at(addr_copy, 16) << 2) + (get_bit_at(addr_copy, 20) << 3);
else
rank = 0;

		int bank_bit0, bank_bit1;
		bank_bit0 = get_bit_at(addr_copy, 17) ^ get_bit_at(addr_copy, 21);
		bank_bit1 = get_bit_at(addr_copy, 18) ^ get_bit_at(addr_copy, 22);
		bank = bank_bit0 + (bank_bit1 << 1);

		// tx_bits have already been sliced off, column bits have already been sliced off... need to slice more
		//slice_lower_bits(addr, 23 - tx_bits - addr_bits[addr_bits.size() - 1]);
	
		//slice_lower_bits(addr, 27 - tx_bits - addr_bits[addr_bits.size() - 1]);
		//slice_lower_bits(addr, 28 - tx_bits - addr_bits[addr_bits.size() - 1]);
		//row = addr;
		row = slice_lower_bits(addr, addr_bits[addr_bits.size() - 2]);//gagan: new fix for row conflicts
	...
	if(promote == false)
	  {
		...
		if(last_request_tick==0)
			trace_entry.off_tick_to_last_request=0;
		else
			trace_entry.off_tick_to_last_request=curTick()-last_request_tick;
		last_request_tick=curTick();
	      	if (req.type == Request::Type::WRITE) 
			trace_entry.is_write_back=true;
		else	
			trace_entry.is_write_back=false;
		trace_entry.block_address=req.addr;
		trace_entry.is_overflow=req.is_overflow;
			
		ofstream write_file_trace;
	        //write_file_trace.open(outputDir+ "trace.cpt", ios::app);
	        write_file_trace.open(outputDir+ trace_name, ios::app);
		write_file_trace.write((char*)&trace_entry, sizeof(trace_entry));
		write_file_trace.close();
	     }
		 //xinw added for generating trace-end
	...
	int column, channel, bank, bankgroup, rank, row, tempA, tempB, channel_mask, channel_mask_temp;
      ...
      int bankgroup_bit0, bankgroup_bit1;
        ...
        return ctrls[req.addr_vec[0]]->get_remaining_queue_size(req);
    }
	...
	int column, channel, bank, bankgroup, rank, row, tempA, tempB, channel_mask, channel_mask_temp;
      ...
      int bankgroup_bit0, bankgroup_bit1;
        ...
        return ctrls[req.addr_vec[0]]->get_overflow_queue_size(req);
    }
	...
	int column, channel, bank, bankgroup, rank, row, tempA, tempB, channel_mask, channel_mask_temp;
      ...
      int bankgroup_bit0, bankgroup_bit1;
        ...
        return ctrls[req.addr_vec[0]]->prioritize(req);
    }

    void set_warmuptime(Tick _atomicwarmuptime, Tick _warmuptime)
	{
		begin_tick=curTick();
		atomicwarmuptime=_atomicwarmuptime;
		warmuptime=_warmuptime;
	}

    void set_simulation_mode(int detailed_mode)
    {
	simulation_mode=detailed_mode;
    }
    void set_timeout(uint64_t _timeout)
    {
	 for (auto ctrl: ctrls)
            ctrl->rowpolicy->timeout=_timeout;
	...
	long write_req = long(incoming_read_reqs_per_channel[ctrl->channel->id].value());
	...
	long prefetch_read_req = long(incoming_prefetch_read_reqs_per_channel[ctrl->channel->id].value());
        ctrl->finish(read_req, write_req, demand_read_req, prefetch_read_req, dram_cycles);
      }
