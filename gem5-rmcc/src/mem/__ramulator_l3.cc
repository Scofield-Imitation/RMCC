...
bool Ramulator::recvTimingReq(PacketPtr pkt) {
    ...
    // if ((curTick() - begin_tick) % print_interval == 0){
    //     std::cout << "ramulator @ tick " << curTick() << ", read count " << my_read_cnt << ", write count "<< my_write_cnt << ", total access " << my_total_cnt << std::endl;
    //     my_read_cnt = 0;
    //     my_write_cnt = 0;
    // }
	...
	bool isHitL3 = L3->call(pkt->getAddr());
	if(isHitL3)
	  {
	    AbstractMemory::numL3CacheHits++;
	  }
	else
	  {
	    AbstractMemory::numL3CacheMisses++;
	  }
        return accepted;
      }
...
void Ramulator::accessAndRespond(PacketPtr pkt) {
	...
	// gagan : added 18 ns latency for the L3 cache
        if (!resp_stall && !send_resp_event.scheduled())
            schedule(send_resp_event, curTick() + 12000);
    } else 
        pending_del.push_back(pkt);
}
